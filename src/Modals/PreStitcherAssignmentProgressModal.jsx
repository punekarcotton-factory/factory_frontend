...
const PreStitcherAssignmentProgressModal = ({
  open,
  onClose,
  assignment,
  onSuccess,
  ...
  const applyQuantityToOption = (option, value) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axiosInstance.patch(
        `/pre-stitchers/assignments/${assignment._id}/progress`,
        {
  ...
  const handleSelectAllRemaining = () => {
    setProgressUpdates((prev) =>
      prev.map((item) => {
        const maxAllowed = item.totalQuantity - item.currentCompleted;
        return { ...item, completedQuantity: maxAllowed };
      })
    );
  };
  ...
  const handleClearAll = () => {
    setProgressUpdates((prev) =>
      prev.map((item) => ({ ...item, completedQuantity: 0 }))
    );
  };
  ...
  const renderQuantityInputs = (groupItems) => {
    return (
      <Stack spacing={3} sx={{ mb: 3 }}>
        {groupItems.map((item) => {
          const currentVal = item.completedQuantity;
          const maxAllowed = item.totalQuantity - item.currentCompleted;
          const inputLabel = getOptionLabel(item.option);

          return (
            <FormControl key={item.option} fullWidth variant="outlined">
              <InputLabel>{inputLabel}</InputLabel>
              <OutlinedInput
                type="number"
                value={currentVal || ""}
                onChange={(e) => applyQuantityToOption(item.option, e.target.value)}
                onWheel={(e) => e.target.blur()}
                label={inputLabel}
                inputProps={{ min: 0, max: maxAllowed }}
                endAdornment={
                  <InputAdornment position="end">
                    <IconButton
                      onClick={() =>
                        applyQuantityToOption(item.option, currentVal - 1)
                      }
                      edge="end"
                      size="small"
                    ...
                    <IconButton
                      ...
                      edge="end"
                      size="small"
                      disabled={currentVal >= maxAllowed}
                    >
                      <Add />
                    </IconButton>
                  </InputAdornment>
              ...
              >
                Available to complete: {maxAllowed} shirts (Completed: {item.currentCompleted}/{item.totalQuantity})
              </Typography>
            </FormControl>
          );
        })}
    ...
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
          ...
          <Box>
            {(() => {
              const groupItems = progressUpdates.filter((item) =>
                PRE_STITCHER_OP_KEYS.includes(item.option),
              );
          ...
          {assignment?.partialCompletions?.length > 0 && (
            <Box sx={{ mt: 1 }}>
              <Typography
                sx={{
                  fontSize: "13px",
              ...
              >
                Previous Notes History
                <Chip
                  label={assignment.partialCompletions.length}
                  size="small"
                    ...
                    >
                      ...
                      >
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                          ...
                          >
                            {entry.receivedBy || "Pre-Stitcher"}
                          </Typography>
                          <Chip
                            label={`+${entry.totalShirtsHandedOver}`}
                            size="small"
                            sx={{
                              height: 18,
                              fontSize: "10px",
...
export default PreStitcherAssignmentProgressModal;
